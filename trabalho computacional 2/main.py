...
coletor_y = -3.0
coletor2_y = -3.0 
COLETOR_MEIA_LARGURA = 1.0
COLETOR_MEIA_ALTURA = 1.0
...
ITEM_MEIA_LARGURA = 1.0 * 0.5
ITEM_MEIA_ALTURA = 1.0 * 0.5
...
def carregar_pontuacoes():
    if not os.path.exists(ARQUIVO_SCORES):
        return {"TEMPO": [], "ZERAR": [], "ENCHER": [], "DISPUTA": []}
    try:
        with open(ARQUIVO_SCORES, "r") as f:
            scores = json.load(f)
            for k in ["TEMPO", "ZERAR", "ENCHER", "DISPUTA"]:
                if k not in scores:
                    scores[k] = []
            return scores
...
class Item:
    def __init__(self):
        ...
        self.y = LIMITE_TOPO
        self.z = 0.0
        self.velocidade = random.uniform(0.03, 0.08)
    ...
    def draw(self, estrela_texture_id):
        glEnable(GL_BLEND)
        ...
        if estrela_texture_id:
            glEnable(GL_TEXTURE_2D)
            glBindTexture(GL_TEXTURE_2D, estrela_texture_id)
...
def desenhar_nave(texture_id, default_color=(0.0, 1.0, 0.0)):
    glEnable(GL_BLEND)
    glEnable(GL_ALPHA_TEST)
    glAlphaFunc(GL_GREATER, 0.1)
    glDisable(GL_CULL_FACE)
...
def resetar_jogo():
    ...
    print("Jogo resetado.")
    return (pontos, lista_itens, tempo_inicio, coletor_x, coletor2_x, pontos_p2, coletor_y, coletor2_y)
...
def main():
    ...
    # imagens / texturas
    texture_coletor_id = carregar_textura("./images/textura_coletor.png")
    texture_coletor_p2_id = carregar_textura("./images/textura_coletor_p2.png")
    estrela_texture_id = carregar_textura("./images/estrela.png")
    skybox_textures = {
    ...
    # rótulos e modos
    botao_labels = [
        f"Modo: Tempo ({OBJETIVO_TEMPO_LIMITE}s)",
        "Modo: Zerar (Nao negative!)",
        f"Modo: Encher ({OBJETIVO_ENCHER_PONTOS} pts)",
        "Modo: Disputa (2P)"
    ]
    botao_modes = ["TEMPO", "ZERAR", "ENCHER", "DISPUTA"]
    ...
    while running:
        ...
        if game_state == ESTADO_MENU:
            ...
            # desce o título
            titulo_y = int(ALTURA_TELA * 0.10)  

            buffer.blit(titulo_img, (int(LARGURA_TELA * 0.05), titulo_y))

            # botões verticais 
            btn_w = int(LARGURA_TELA * 0.45)
            btn_h = int(ALTURA_TELA * 0.10)
            espaco = int(ALTURA_TELA * 0.03)
            base_x = int(LARGURA_TELA * 0.05)
            base_y = int(ALTURA_TELA * 0.28)
            ...
            if pygame.mouse.get_pressed()[0]:
                for idx, r in enumerate(botoes_rects):
                    if r.collidepoint(mouse_pos):
                        ...
                        pontos, lista_itens, tempo_inicio, coletor_x, coletor2_x, pontos_p2, coletor_y, coletor2_y = resetar_jogo()
                        break
        ...
        # ------------ JOGANDO ------------
            ...
            # clear depth so scene draws on top
            ...
            for item in lista_itens:
                item.draw(estrela_texture_id)

            glPushMatrix()
            glTranslatef(coletor_x, coletor_y, 0.0) 
            desenhar_nave(texture_coletor_id, default_color=(0.0, 1.0, 0.0))
            glPopMatrix()

            if modo_de_jogo == "DISPUTA":
                glPushMatrix()
                glTranslatef(coletor2_x, coletor2_y, 0.0) 
                desenhar_nave(texture_coletor_p2_id, default_color=(1.0, 0.5, 0.0))
                glPopMatrix()
            ...
            if modo_de_jogo == "DISPUTA":
                desenhar_texto(f"P2 Pontos: {pontos_p2}", fonte_placar, LARGURA_TELA - 220, ALTURA_TELA - 40,
                               (255, 255, 255, 255))
                tempo_restante = max(0, int(OBJETIVO_TEMPO_LIMITE - tempo_atual_seg))
